	...
	public function field_preview_option( $option, $field, $args = array(), $echo = true ) {
		...
		switch ( $option ) {
			...
			case 'decision':
				...
				$output  = sprintf( '<label class="label-title %s"><input type="checkbox" %s disabled><span class="text">%s</span><span class="required">*</span></label>', $class, checked( '1', $default, false ), $label );
				break;
			case 'choices':
				$values  = ! empty( $field['choices'] ) ? $field['choices'] : $this->defaults;
				...
				if ( in_array( $field['type'], array( 'checkbox', 'payment-multiple', 'radio' ), true ) ) {
					...
					foreach ( $values as $key => $value ) {
						...
						$output .= sprintf( '<li class="%s">',
							evf_sanitize_classes( $item_class, true )
						);
				...
				// Dynamic population is enabled and contains more than 20 items, include a note about results displayed.
				if ( $total && $total > 20 ) {
					$output .= '<div class="everest-forms-alert-dynamic everest-forms-alert everest-forms-alert-warning">';
					/* translators: %d - total amount of choices. */
					$output .= sprintf(
						wp_kses(
							__( 'Showing the first 20 choices.<br> All %d choices will be displayed when viewing the form.', 'everest-forms' ),
							array(
								'br' => array()
							)
						),
